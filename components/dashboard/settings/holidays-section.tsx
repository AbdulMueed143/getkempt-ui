...
export function HolidaysSection({ form }: Props) {
  ...
  return (
    <div id="holidays" className="divide-y divide-[#F0EEE6]">
      {/* Action bar */}
      <div className="px-5 sm:px-6 py-4 flex items-center gap-2 flex-wrap">
        <button
          type="button"
          onClick={handleAddVicHolidays}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-[#0D1B2A] bg-gradient-to-br from-[#F8F6F2] to-[#F5F3EE] border border-[#E8E4DA] hover:shadow-sm hover:border-[#C4A882]/40 transition-all duration-200"
        >
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative w-28">
                {draft.surchargeType === "fixed_aud" && (
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-[#6B7280] pointer-events-none">$</span>
                )}
                <input
                  type="number"
                  min={0}
                ...
                {draft.surchargeType === "percentage" && (
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-[#6B7280] pointer-events-none">%</span>
                )}
              </div>
            </div>
          )}
      ...
      {fields.length === 0 ? (
        <div className="py-12 text-center">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#F8F6F2] to-[#F5F3EE] flex items-center justify-center mx-auto mb-3">
            <CalendarOff className="w-7 h-7 text-[#C4C9D4]" />
          </div>
          <p className="text-sm text-[#6B7280] font-medium">No holidays added yet</p>
                ...
                <li key={field.id} className="px-5 sm:px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-[#6B7280] mt-0.5">{formatDate(field.date)}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        className="p-2 rounded-xl text-[#6B7280] hover:text-[#6B7A99] hover:bg-[#F5F3EE] transition-all duration-200"
                      >
                  ...
                  {/* Inline edit panel */}
                  {isExpanded && (
                    <div className="mt-4 ml-17 pl-3 border-l-2 border-[#E8E4DA] space-y-3">
                      <div className="grid sm:grid-cols-2 gap-3">
                        <div className="space-y-1.5">
