...
export function BookingsPageClient() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] min-h-0 -mt-1">
      ...
      {/* ── Calendar nav bar ── */}
      <div className="flex items-center gap-2 px-1 py-2 shrink-0 border-b border-[#E8ECF4]">
      ...
      {/* ── Mobile today stats ── */}
      <div className="flex lg:hidden items-center gap-3 px-2 py-1.5 text-[10px] shrink-0 overflow-x-auto">
        <span className="text-[#8E95A5] font-medium shrink-0">Today:</span>
        <span className="flex items-center gap-1 text-[#0D1B2A] shrink-0"><CheckCircle2 className="w-3 h-3" />{stats.confirmed} confirmed</span>
        <span className="flex items-center gap-1 text-emerald-600 shrink-0"><CheckCircle2 className="w-3 h-3" />{stats.completed} done</span>
        <span className="flex items-center gap-1 text-rose-500 shrink-0"><XCircle className="w-3 h-3" />{stats.noShow} no-show</span>
      </div>

      {/* ── Calendar body ── */}
      <div className="flex-1 min-h-0 overflow-hidden bg-white rounded-xl border border-[#E8ECF4] mt-1">
        {view === "month" && (
          <MonthView
            currentDate={curDate}
            bookings={bookings}
